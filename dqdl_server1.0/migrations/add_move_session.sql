...
CREATE TABLE IF NOT EXISTS `move_session` (
  ...
  `current_net_id` INT NOT NULL COMMENT '当前已到达节点 id（多段移动进度）',
  `current_seg` INT NOT NULL DEFAULT 0 COMMENT '当前段索引（0起）',
  `from_name` VARCHAR(64) DEFAULT NULL COMMENT '起点名（快照）',
  `to_name` VARCHAR(64) DEFAULT NULL COMMENT '终点名（快照）',
  `line` TEXT DEFAULT NULL COMMENT '移动路径 JSON:[{id,name,gx,gy}]（大地图画变色线用）',
