 ...
 *   - 锁玩家状态(SCRIPT) + 同步选角（DynamicNpcService.acquire）
 *   - 写 ScriptInstance（pending + current_node=start + actor_mapping）
...
@Module({
  imports: [
    TypeOrmModule.forFeature([ScriptOutline, ScriptInstance]),
    PlayerModule,
    NpcModule,
  ],
