...
export class EncounterService {
  ...
  /** 会话结束：把对应奇遇标记为已完成（从列表移除） */
  markDone(encounterId: number | null, playerId: number): Promise<any> {
    if (!encounterId) return Promise.resolve(null);
    return this.repo.update({ id: encounterId, player_id: playerId }, { status: 'done' });
  }

  /** 放弃奇遇（仅 pending 可放弃） */
  async abandon(id: number, playerId: number): Promise<void> {
    await this.repo.update({ id, player_id: playerId, status: 'pending' }, { status: 'abandoned' });
  }

  /** 进入奇遇：校验属主+pending，返回奇遇数据并标记为已进入（由 dungeon.enter 调用） */
  async consume(id: number, playerId: number): Promise<Encounter | null> {
    const enc = await this.repo.findOne({ where: { id, player_id: playerId, status: 'pending' } });
    if (!enc) return null;
    enc.status = 'entered';
