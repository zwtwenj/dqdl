...
# (role_name, text, event, visible_rule)
# event 约定：裸字符串事件名（如 'createAdventurerTask'），与前端 dialogEventHandlers
#   注册表的 key 一对一匹配。空串 = 纯对话氛围按钮，点击无副作用。
# visible_rule 约定：可见性规则函数名（dialog-visible-rules 中注册）。
#   空串 = 始终显示；非空 = 按规则函数返回值过滤（如 hasClaimableTask = 有可交付任务才显示）。
events = [
    # 公会接待员 (role_id=1)
    ('公会接待员', '我想要接取一些任务', 'createAdventurerTask', ''),
    ('公会接待员', '我完成了任务，来交付', 'completeTask', 'hasClaimableTask'),
    ('公会接待员', '这里有什么难度的任务？', '', ''),
    ('公会接待员', '最近有什么特别的悬赏吗？', '', ''),
...
count = 0
for role_name, text, event, visible_rule in events:
    role_id = role_map.get(role_name)
    if not role_id:
        print(f'  SKIP {role_name} (not found)')
        continue
    cur.execute(
        "INSERT INTO dialog_event (role_id, text, event, visible_rule) VALUES (%s, %s, %s, %s)",
        (role_id, text, event, visible_rule)
    )
    count += 1
